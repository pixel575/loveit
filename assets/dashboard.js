// assets/dashboard.js - improved: search, logout binding, and minor UX tweaks
(function(){
  ...
  function renderCardHTML(a, showDownload){ const hasTrailer = !!(a.trailer && a.trailer.site && a.trailer.id); return `<img src="${a.img}" alt="${a.title}"><div class="meta"><h3>${a.title}</h3><p>${a.year} • ${a.episodes} eps • ⭐ ${a.score}</p><div class="btn-row"><button class="btn" data-id="${a.id}" data-action="details">Details</button><button class="btn-plain" data-id="${a.id}" data-action="watch">Add to Watchlist</button></div></div><div class="overlay-controls">${hasTrailer?'<button class="play-btn" data-action="play" data-id="'+a.id+'" aria-label="Play trailer for '+a.title+'"><svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 3v18l15-9L5 3z" fill="#081026"/></svg></button>':''}<a class="download-btn" ${showDownload?`data-action="download" data-id="${a.id}"`:'href="#" style="opacity:0.5;pointer-events:none;"'} aria-label="Download clip for ${a.title}"><svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 3v10" stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M8 11l4 4 4-4" stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M21 21H3" stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></a><a class="download-btn" href="${a.anilistUrl}" target="_blank" rel="noopener" style="margin-left:8px">AniList</a></div>`; }
